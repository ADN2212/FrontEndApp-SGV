function estaEnUso(
	...
	*/
	...
	let arrayValores = arrayObjetos.map((objeto) => {
		return objeto[llaveAEvaluar];
	});

	if (esEditar) {
		//En caso de que se este editando hay que obviar el valor anterior:
		arrayValores = arrayValores.filter((v) => {
			return v !== valorAnterior;
		});
	}
	...
	let estaValor = !(
		arrayValores.filter((v) => {
			return v === valorActual;
		}) == false
	);
