...
function App() {
  ...
  */
  ...
  const [valorFiltrarChoferes, setValorFiltrarChoferes] = useState({'clasificacion': null});
  const [valorFiltrarVehiculos, setValorFiltrarVehiculos] = useState({'tipo_servicio' : null});
  const [valorFiltrarViajes, setValorFiltrarViajes] = useState({'estado_viaje': null});
  ...
  useEffect(() => {getAllData('sgv_user', setUsuarios)}, []);

  useEffect( () => {loginAtStart(setActiveUser)}, []);

  //useEffect(()=> {console.log(valorFiltrarUsers)})
  ...
  */
  ...
  if (tipoObjeto === 'chofere' || tipoObjeto === 'vehiculo' || tipoObjeto === 'viaje' || 'sgv_user'){  
  ...
  async function postData(json, tipoObjeto){
    ...
    */
    ...
    if (tipoObjeto === 'chofer' || tipoObjeto === 'vehiculo' || tipoObjeto === 'viaje' || tipoObjeto === 'sgv_user'){  
    ...
    try {  
        ...
        if (response.ok){
          ...
          return;
            ...
            title: `Error`,
            icon: 'error',
            text: `Ha ocurrido un error: ${response.status}`
          });*/
          refreshToken();
    ...
    */
    ...
    try {
      ...
      if (response.ok){
        ...
        swal({
          title: `${tipoObjeto === 'sgv_user' ? 'Usuario' : tipoObjeto} Eliminado`, 
          icon: 'success',
          text: `El ${tipoObjeto === 'sgv_user' ? 'Usuario' : tipoObjeto} de id = ${id} ha sido borrado exitosamente!`
        });
          ...
          if (response.ok){            
            ...
            return;
              ...
              swal({
                title: 'Error',
                icon: 'error',
                text: errorData.status = 401 ? 'No autorizado.' : ` Ha ocurrido un error: ${errorData.status}.`
              })*/
              refreshToken();
  ...
  //<h1 className = 'display-2 text-primary'> Let’s Go Bus </h1>
      ...
      <Router>
        ...
        <Routes>
          ...
          <Route path = 'login' element = { 
            ...
            : 
              <FormaLogout activeUser = {activeUser} setActiveUser = {setActiveUser} />
                  ...
                  { 
                    isLogin(activeUser) ?
                      <ListaObjetos
                          ...
                          objetosAMostrar = { 
                                            filtrarObjetos(choferes, valorFiltrarChoferes.clasificacion, 'clasificacion')
                                            } 
                          tipoObjeto = {'chofer'} 
                          arrayViajes = {viajes}
          ...
          <Route path = '/vehiculos' element = 
                  {
                    isLogin(activeUser) ? 
                      <ListaObjetos
                          objetosAMostrar = {filtrarObjetos(vehiculos, valorFiltrarVehiculos.tipo_servicio, 'tipo_servicio')} 
          ...
          <Route path = '/viajes' element = 
                    {
                      isLogin(activeUser) ?
                        activeUser.rol === 'Administrador' || activeUser.rol === 'Agente' ?
                          <ListaObjetos
                              objetosAMostrar = {filtrarObjetos(viajes, valorFiltrarViajes.estado_viaje, 'estado_viaje')}  
          ...
          <Route path = '/usuarios' element = 
                {
                  isLogin(activeUser) ? 
                    activeUser.rol === 'Administrador' ?
                      <ListaObjetos 
                          objetosAMostrar = {filtrarObjetos(usuarios, valorFiltrarUsers.rol, 'rol')} 
          ...
          <Route path = '/calcular_ganacias' element = {
