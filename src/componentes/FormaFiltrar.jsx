...
export default function FormaFiltrar({tipoObjeto, setterFunc, cambiarEstado}){
	//Muestra una forma que servira para filtart los objetos por sus campos de seleción.
	...
	return(
		...
		<form onSubmit = {(evento) => {cambiarValorFiltrar(evento, tipoObjeto)}} className = 'AnchoForma m-3 bg-dark p-3 border border-primary border-3 rounded-5 text-white'>
			<h1 className = 'display-6 text-primary'> {`Filtar ${plural}`} </h1>
			{
				tipoObjeto ===  'usuario' ? 
					<label className = 'form-label'>
						Selecione Tipo de Usuario:
						<select className = 'form-select form-select-sm' name = 'rol' value = {valorFiltrar.rol || ""} onChange = {(evento) => {alCambiar(evento, valorFiltrar, setValorFiltrar)}}>
				...
				: tipoObjeto === 'chofer' ?
						<label className = 'form-label'>
							...
							<select className = 'form-select form-select-sm' name = 'clasificacion' value = {valorFiltrar.clasificacion || ""} onChange = {(evento) => {alCambiar(evento, valorFiltrar, setValorFiltrar)}}>
								<option value = 'Todos'> Todos </option>
				...
				: tipoObjeto === 'vehiculo' ?
						<label className = 'form-label'>
							...
							<select className = 'form-select form-select-sm' name = 'tipo_servicio' value = {valorFiltrar.tipo_servicio || ""} onChange = {(evento) => {alCambiar(evento, valorFiltrar, setValorFiltrar)}}>
								<option value = 'Todos'> Todos </option>
				...
				: tipoObjeto === 'viaje' ?
					<label className = 'form-label'>
						Selecione el Estado de los Viajes:
							<select className = 'form-select form-select-sm' name = 'estado_viaje' value = {valorFiltrar.estado_viaje || ""} onChange = {(evento) => {alCambiar(evento, valorFiltrar, setValorFiltrar)}}>
							<option value = 'Todos'> Todos </option>
