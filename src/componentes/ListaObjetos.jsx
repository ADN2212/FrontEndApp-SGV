		...
		if (mostrarFormaFiltar) {
			return (
				<FormaFiltrar
					...
					cambiarEstado={() => {
						cambiarEstado(mostrarFormaFiltar, setMostrarFormaFiltar);
					}}
				/>
			);
		}
		...
		<div className='m-5 mt-3'>
			{activeUser ? (
				<center>
					<h4 className='text-primary m-3'>
						<u>
							{' '}
							{`Usuario Activo: ${activeUser['nombre_completo']}, rol: ${activeUser['rol']}`}{' '}
						</u>
					</h4>
				</center>
			) : (
			...
			<div className='container'>
				<button
					type='button'
					className={
						!mostrarForma ? 'btn btn-primary m-1' : 'btn btn-secondary m-1'
					}
					onClick={() => {
						setMostrarForma(!mostrarForma);
					}}>
					{!mostrarForma ? (
						<h5> Click para agregar un {tipoObjeto} </h5>
					) : (
						<h5> Cerrar </h5>
					)}
				</button>
				...
				<button
					...
					onClick={() => {
						setMostrarFormaFiltar(!mostrarFormaFiltar);
					}}>
			...
			<div className='listado'>
				{arrayObjetos.length === 0 ? (
					<p className='text-danger'>
						{' '}
						No hay {plural}, o el servidor está apagado, verifique que este
						ultimo esté funcionando para poder ver los {plural}, recargue la
						pagina luego de haberlo ensendido.{' '}
					</p>
				) : tipoObjeto === 'chofer' ? (
					objetosAMostrar.map((chofer) => {
						return (
							<Chofer
