				...
				if (jsonTipoViaje['estado_viaje'] === 'Cerrado') {
					...
					return (
						<p className='text-warning'>
							{' '}
							<b> Los viajes que han sido cerrados no son editables. </b>{' '}
						</p>
					);
				}

				if (
					jsonTipoViaje['estado_viaje'] === 'En Proceso' &&
					(!chofer || !vehiculo)
				) {
					return (
						<p>
							<small>
								<b>
									El chofer o el vehiculo (o ambos) asiganado a este viaje ha
									sido borrado antes de que este se culminara, recomendamos que
									el viaje sea eliminado y que luego se cree uno nuevo con los
									mismos parametros del anterior pero con un chofer o viaje que
									todavia pertenezca a la Base de Datos del sistema.
								</b>
							</small>
						</p>
					);
	...
	return (
		<div className='card m-3 mt-3 ps-5 pt-1 bg-dark border border-primary border-3 rounded-5'>
			<h2 className='display-6 text-primary'>
				Viaje de Tipo {jsonTipoViaje['tipo_viaje']}
			</h2>
			<ul className='list-unstyled text-body'>
				<li className='text-white'> ID: {jsonTipoViaje['id_viaje']} </li>
				<li className='text-white'>
					{' '}
					Fecha de Inicio: {parseFecha(jsonTipoViaje['inicio_viaje'])}{' '}
				</li>
				<li className='text-white'>
					{' '}
					Fecha de Termino: {parseFecha(jsonTipoViaje['fin_viaje'])}{' '}
				</li>
				<li className='text-white'>
					{' '}
					Costo: {jsonTipoViaje['costo_usd']} US${' '}
				</li>
				<li className='text-white'>
					{' '}
					Distancia Recorrida: {jsonTipoViaje['kms_recorridos']} Kilometros{' '}
				</li>
				<li className='text-white'>
					{' '}
					Estado: {jsonTipoViaje['estado_viaje']}{' '}
				</li>
				<li className={chofer ? 'text-white' : 'text-danger'}>
					{chofer
						? `Chofer de Nombre: ${chofer['nombre_completo']} y cedula: ${chofer['cedula']}`
						: 'Chofer Eliminado'}
				</li>
				...
				<li className='text-white'>
					{' '}
					Comentario: {jsonTipoViaje['comentarios']}{' '}
				</li>
				<li className='text-white'>
					<b>Opcines:</b>
					<ol className='list-inline'>
						...
						<li className={!mostrarOpcionesEliminar ? `list-inline-item` : ''}>
							<button
								...
								onClick={() => {
									setMostrarOpcionesEliminar(!mostrarOpcionesEliminar);
								}}>
