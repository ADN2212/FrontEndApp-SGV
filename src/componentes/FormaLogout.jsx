...
export default function FomaLogout({activeUser, setActiveUser}){
	...
	return (
		<div className = 'p-3'>
			<center>
				<form onSubmit = {(evento) => {logOutAndBlacklistTokens(evento)}} className = 'm-3 bg-dark AnchoForma p-3 border border-primary border-3 rounded-5'>
					<h2 className = 'text-warning display-5'> Logout </h2>
					<h3 className = 'lead text-primary'> {`Actualmente está logeado como el/la ${activeUser.rol}/a de nombre ${activeUser.nombre_completo}.`} </h3>
