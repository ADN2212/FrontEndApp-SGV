...
export default function FormaViaje({
	...
	function EnviarData(evento) {
		...
		//Analizar las fechas de inicio y fin del viaje:
		...
		if (deltaTime < 3600000) {
			//1 hora = 60 mins = 3,600 segs = 3,600,000 milisegs.
			swal({
		...
		viajeToSend['vehiculo'] = {
			id_vehiculo: parseInt(viajeData['id_vehiculo']),
		};
		...
		if (viajesIntesectados) {
			...
			swal({
				...
				icon: 'error',
				text: `Las fechas de inicio y fin de este viaje generan conflictos con las de los siguientes viajes: ${ids}.`,
			});
		...
		if (viajeToSend['estado_viaje'] === 'Cerrado') {
			...
			let vehiculoActualizar = arrayVehiculos.filter((vehiculo) => {
				return (
					vehiculo['id_vehiculo'] === viajeToSend['vehiculo']['id_vehiculo']
				);
			...
			putFunc(
				vehiculoActualizar['id_vehiculo'],
				'vehiculo',
				vehiculoActualizar,
				true,
			);
			...
			let choferActualizar = arrayChoferes.filter((chofer) => {
	...
	return (
		<form
			...
			className={
				esEditar
					? ''
					: 'm-3 bg-dark AnchoForma p-3 border border-primary border-3 rounded-5 text-white'
			}>
			...
			<label className='form-label'>
				...
				{!(
					esEditar &&
					(viajeData['estado_viaje'] === 'Cerrado' ||
						viajeData['estado_viaje'] === 'En Proceso')
				) ? (
					<select
						className='form-select form-select-sm'
						name='tipo_viaje'
						value={viajeData.tipo_viaje || ''}
						onChange={(evento) => {
							alCambiar(evento, viajeData, setViajeData);
						}}>
			...
			<label className='form-label'>
				...
				{!(
					...
					!(arrayChoferes.length === 0) ? (
							...
							{arrayChoferes.map((chofer) => {
								return (
									<option
										...
										value={
											chofer.id_chofer
										}>{`Nombre: ${chofer.nombre_completo}, Cedula: ${chofer.cedula}`}</option>
								);
							})}
						</select>
					) : (
						<b>
							{' '}
							No hay choferes, por favor agregue uno para poder crear viajes.{' '}
						</b>
					)
				) : (
					<>
						{viajeData['chofer']
							? ` Nombre: ${viajeData['chofer']['nombre_completo']} y cedula: ${viajeData['chofer']['cedula']}`
			...
			<label className='form-label'>
				...
				{!(
					...
					!(arrayVehiculos.length === 0) ? (
							...
							{arrayVehiculos.map((vehiculo) => {
								return (
									<option
										...
										{' '}
										{`Vehiculo de Marca: ${vehiculo.marca} y Placa: ${vehiculo.placa}`}{' '}
									</option>
								);
							})}
						</select>
						...
						<b>
							{' '}
							No hay vehiculos disponibles, por favor agregue uno, o espere que
							uno esté disponible para poder crear viajes.{' '}
						</b>
					)
				) : (
					<>
						{viajeData['vehiculo']
							? ` Vehiculo de Marca: ${viajeData['vehiculo']['marca']} y Placa: ${viajeData['vehiculo']['placa']}`
							: ' El vehiculo a sido borrado.'}{' '}
					</>
				)}
			</label>
			<br></br>
			...
			<input
				type='submit'
				value={esEditar ? 'Editar Viaje' : 'Crear Viaje'}
