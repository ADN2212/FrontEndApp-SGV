...
export default function FormaChofer({
	arrayChoferes,
	postFunc,
	putFunc,
	jsonTipoChofer,
	...
	function EnvirarData(evento) {
		...
		if (
			estaEnUso(data.cedula, arrayChoferes, esEditar, 'cedula', cedulaAnteiror)
		) {
			//Ademas de esta se pueden probar que se complan otras condiciones antes de que se haga la llamada a la API.
			swal({
				title: 'Cedula en Uso',
				icon: 'warning',
			...
			estaEnUso(data.email, arrayChoferes, esEditar, 'email', emailAnterior)
		) {
			swal({
				title: 'Email en Uso',
				icon: 'warning',
	...
	return (
		<form
			...
			<input
				...
				value={esEditar ? 'Editar Chofer' : 'Crear Chofer'}
			/>
		</form>
	);
}